...
//    test("use derby");
//    test("use `drill_derby_test`");
//    test("use `derby`.`drill_derby_test`");
  }

  @Test
  public void testCrossSourceMultiFragmentJoin() throws Exception {
    testNoResult("SET `planner.slice_target` = 1");
    String query = String.format("select x.person_id, y.salary from derby.drill_derby_test.person x "
  ...
  public void validateResult() throws Exception {
    ...
    testBuilder()
            ...
            .baselineValues(1, "first_name_1", "last_name_1", "1401 John F Kennedy Blvd",   "Philadelphia",     "PA",
                            19107, "{ a : 5, b : 6 }",            123456L,         1, new BigDecimal("10.01"), false, 1.0, 1.1, 111.00,
            ...
            .baselineValues(2, "first_name_2", "last_name_2", "One Ferry Building",         "San Francisco",    "CA",
                            94111, "{ foo : \"abc\" }",            95949L,         2, new BigDecimal("20.02"), true, 2.0, 2.1, 222.00,
            ...
            .baselineValues(3, "first_name_3", "last_name_3", "176 Bowery",                 "New York",         "NY",
                            10012, "{ z : [ 1, 2, 3 ] }",           45456L,        3, new BigDecimal("30.04"), true, 3.0, 3.1, 333.00,
            ...
            .baselineValues(4, null, null, "2 15th St NW", "Washington", "DC", 20007, "{ z : { a : 1, b : 2, c : 3 } " +
                    "}", -67L, 4, new BigDecimal("40.04"), false, 4.0, 4.1, 444.00, DateUtility.parseLocalTime("16:00:01.0"), DateUtility.parseLocalDateTime("2015-06-01 16:00:01.0"),  DateUtility.parseLocalDate("2015-06-01"), "xxx")
  ...
  public void pushdownDoubleJoinAndFilter() throws Exception {
    ...
    final String minimalQueryForReproduce = "select * from \n" +
        "derby.drill_derby_test.person e\n" +
        "INNER JOIN \n" +
        "derby.drill_derby_test.person s\n" +
        "ON e.person_ID = s.person_ID\n" +
        "WHERE e.first_name > 'abc' and s.first_name > 'efg'";
    // NOTE: the query with JOIN statements and filter on one the only side of the join works fine
    // "WHERE e.first_name > 'abc' and e.first_name > 'efg'";
    testPlanMatchingPatterns(minimalQueryForReproduce, new String[] {}, new String[] { "Join", "Filter" });
  }
