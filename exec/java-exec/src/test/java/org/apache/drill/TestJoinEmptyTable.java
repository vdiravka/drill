...
public class TestJoinEmptyTable extends BaseTestQuery {
  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(TestJoinEmptyTable.class);
  ...
  public void testHashJoinWithLeftEmptyDirTable() throws Exception {
    try {
      ...
      String query = String.format("select t1.`employee_id`, t1.`full_name`, t2.`employee_id`, t2.`full_name` " +
          "from dfs.tmp.`%s` t1 inner join cp.`employee.json` t2 on t1.`full_name` = t2.`full_name`", emptyDirName);
  ...
  public void testHashJoinWithRightEmptyDirTable() throws Exception {
    try {
      ...
      String query = String.format("select t1.`employee_id`, t1.`full_name`, t2.`employee_id`, t2.`full_name` " +
          "from cp.`employee.json` t1 inner join dfs.tmp.`%s` t2 on t1.`full_name` = t2.`full_name`", emptyDirName);
      final int expectedRecordCount = 0;
  ...
  public void testHashJoinWithBothEmptyDirTables() throws Exception {
    try {
      ...
      String query = String.format("select t1.`employee_id`, t1.`full_name`, t2.`employee_id`, t2.`full_name` " +
          "from dfs.tmp.`%1$s` t1 inner join dfs.tmp.`%1$s` t2 on t1.`full_name` = t2.`full_name`", emptyDirName);
      final int expectedRecordCount = 0;
  ...
  public void testLeftHashJoinEmptyDirTable() throws Exception {
    try {
      ...
      String query = String.format("select t1.`employee_id`, t1.`full_name`, t2.`employee_id`, t2.`full_name` " +
          "from cp.`employee.json` t1 left join dfs.tmp.`%s` t2 on t1.`full_name` = t2.`full_name`", emptyDirName);
