...
public class TestSelectWithFullNameIdentifiers extends BaseTestQuery {
  ...
  public static void createTableForTest() throws Exception {
    test("USE dfs.tmp");
    test("CREATE TABLE `department` (department_id, department_description) as select department_id, department_description from cp.`department.json`");
  }

  // delete table from dfs
  @AfterClass
  public static void dropCreatedTable() throws Exception {
    test("DROP TABLE IF EXISTS `department`");
  }
...
//  @Ignore
  ...
  public void testLeftJoin() throws Exception {
    ...
    testBuilder()
        .sqlQuery("SELECT cp.`employee.json`.employee_id, cp.`employee.json`.full_name, dfs.tmp.`department`.department_id " +
            "FROM cp.`employee.json` LEFT JOIN dfs.tmp.`department` " +
            "ON cp.`employee.json`.department_id = dfs.tmp.`department`.department_id " +
            "WHERE dfs.tmp.`department`.department_description = 'HQ Marketing' " +
            "ORDER BY cp.`employee.json`.full_name")
        .unOrdered()
  ...
  public void testNestedQueryInWhereStatement() throws Exception {
    ...
    testBuilder()
        .sqlQuery("SELECT cp.`employee.json`.employee_id, cp.`employee.json`.department_id " +
            ...
            "(SELECT dfs.tmp.`department`.department_id " +
            "FROM dfs.tmp.`department` " +
            "WHERE dfs.tmp.`department`.department_id > 0)")
        .unOrdered()
  ...
  public void testCorrelatedSubQuery() throws Exception {
    ...
    testBuilder()
        .sqlQuery("SELECT dfs.tmp.`department`.department_id, " +
            "(SELECT COUNT(cp.`employee.json`.employee_id) FROM cp.`employee.json` " +
            "WHERE cp.`employee.json`.department_id = dfs.tmp.`department`.department_id) count_employee " +
            "FROM dfs.tmp.`department` " +
            "WHERE dfs.tmp.`department`.department_id = 1")
        .unOrdered()
