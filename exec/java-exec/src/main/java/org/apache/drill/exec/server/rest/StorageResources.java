...
public class StorageResources {
  ...
  @GET
  @Path("/storage/{name}/export/{fileType}")
  @Produces(MediaType.APPLICATION_JSON)
  public Response exportPlugin(@PathParam("name") String name, @PathParam("fileType") String fileType) {
    if (JSON_FILE_NAME.equals(fileType) || HOCON_FILE_NAME.equals(fileType)) {
      ...
      response.header("Content-Disposition", String.format("attachment;filename=\"%s.%s\"", name, fileType));
      return response.build();
    }
  ...
  @DELETE
  @Path("/storage/{name}.{fileType}")
  @Produces(MediaType.APPLICATION_JSON)
  public JsonResult deletePlugin(@PathParam("name") String name, @PathParam("fileType") String fileType) {
    if (JSON_FILE_NAME.equals(fileType) || HOCON_FILE_NAME.equals(fileType)) {
      PluginConfigWrapper plugin = getPluginConfigs(name);
    ...
    return message("Error (unable to delete %s.%s storage plugin)", name, fileType);
  }
