...
public class ParquetGroupScan extends AbstractFileGroupScan {
  ...
  initFromMetadataCache(FileSelection selection, Path metaFilePath) throws IOException {
    ...
    if (fileStatuses.size() == 1 && selection.getSelectionRoot().equals(first.toString())) {
      ...
      for (Metadata.ParquetFileMetadata file : parquetTableMetadata.getFiles()) {
        fileSet.add(file.getParquetPath().getFullPath());
      }
      ...
      if (selection.wasAllPartitionsPruned()) {
        ...
        fileSet.add(this.parquetTableMetadata.getFiles().get(0).getParquetPath().getFullPath());
      } else {
    ...
    for (ParquetFileMetadata file : parquetTableMetadata.getFiles()) {
      ...
      for (RowGroupMetadata rg : file.getRowGroups()) {
        RowGroupInfo rowGroupInfo =
            new RowGroupInfo(file.getParquetPath().getFullPath(), rg.getStart(), rg.getLength(), rgIndex, rg.getRowCount());
    ...
    for (ParquetFileMetadata file : parquetTableMetadata.getFiles()) {
      for (RowGroupMetadata rowGroup : file.getRowGroups()) {
        ...
        for (ColumnMetadata column : rowGroup.getColumns()) {
          ...
          if (partitionColumn) {
            Map<SchemaPath,Object> map = partitionValueMap.get(file.getParquetPath().getFullPath());
            if (map == null) {
              map = Maps.newHashMap();
              partitionValueMap.put(file.getParquetPath().getFullPath(), map);
            }
  ...
  private ParquetTableMetadataBase removeUnneededRowGroups(ParquetTableMetadataBase parquetTableMetadata) {
    ...
    for (ParquetFileMetadata file : parquetTableMetadata.getFiles()) {
      if (fileSet.contains(file.getParquetPath().getFullPath())) {
  ...
  public GroupScan applyFilter(LogicalExpression filterExpr, UdfUtilities udfUtilities,
    ...
    for (ParquetFileMetadata file : parquetTableMetadata.getFiles()) {
      ...
      Map<String, String> implicitColValues = columnExplorer.populateImplicitColumns(file.getParquetPath().getFullPath(), selectionRoot);
      ...
      for (RowGroupMetadata rowGroup : file.getRowGroups()) {
        ...
        qualifiedFileNames.add(file.getParquetPath().getFullPath());  // TODO : optimize when 1 file contains m row groups.
      }
    }
