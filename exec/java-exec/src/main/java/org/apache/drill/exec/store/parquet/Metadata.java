...
//    Map<SchemaPath, OriginalType> originalTypeMap = Maps.newHashMap();
  ...
  private void readBlockMeta(Path path, boolean dirsOnly, MetadataContext metaContext) {
    Stopwatch timer = Stopwatch.createStarted();
    Path metadataParentDirPath = Path.getPathWithoutSchemeAndAuthority(path.getParent());
    String metadataParentDirStringPath = metadataParentDirPath.toUri().getPath();
    ...
    try (FSDataInputStream is = fs.open(path)) {
      ...
      boolean newMetadata = false;
        alreadyCheckedModification = metaContext.getStatus(metadataParentDirStringPath);
      ...
      if (dirsOnly) {
        ...
        timer.stop();
        parquetTableMetadataDirs.updateRelativePaths(metadataParentDirStringPath);
        if (!alreadyCheckedModification && tableModified(parquetTableMetadataDirs.getDirectories(), path, metadataParentDirPath, metaContext)) {
        ...
        if (new MetadataVersion(parquetTableMetadata.getMetadataVersion()).compareTo(new MetadataVersion(3, 0)) >= 0) {
          ((ParquetTableMetadata_v3) parquetTableMetadata).updateRelativePaths(metadataParentDirStringPath);
        }
        if (!alreadyCheckedModification && tableModified(parquetTableMetadata.getDirectories(), path, metadataParentDirPath, metaContext)) {
      ...
      metaContext.setMetadataCacheCorruptionStatus(true);
    }
  }
